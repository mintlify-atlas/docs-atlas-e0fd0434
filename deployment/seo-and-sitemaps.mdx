---
title: "SEO, sitemaps, and structured data configuration"
sidebarTitle: "SEO & Sitemaps"
description: "Real Estate Template auto-generates JSON-LD structured data, a dynamic sitemap.xml, and robots.txt. Set NUXT_PUBLIC_SITE_URL to enable all absolute-URL SEO features."
---
...
`usePageSeo()` lives in `app/core/composables/usePageSeo.ts`. It is **not** auto-imported — every page that needs it must import it explicitly, which makes call sites easy to audit.
...
| `/agents` | `ItemList` of `Person` | Each agent's `worksFor` references the agency home page `@id` |
