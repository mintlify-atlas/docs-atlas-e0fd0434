---
title: "propertiesService: filtering, sorting, and lookup"
sidebarTitle: "propertiesService"
description: "propertiesService is the single data access layer for the property catalog. Methods cover listing, slug lookup, filtered queries, featured sets, and related properties."
---
...
```ts
filter(filters: PropertyFilters, sort?: PropertySort): Property[]
```
...
```ts
getRelated(current: Property, limit?: number): Property[]
```

Finds properties similar to `current` using a weighted scoring model over the visible catalog. The result is used by the related-properties section on the property detail page. Sold, rented, hidden, and reserved-but-zero-score properties are excluded. The `current` property itself is always excluded from results.
